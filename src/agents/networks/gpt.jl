...
@layer :noexpand _TransformerBlock
...
function _TransformerBlock(
    ...
    return _TransformerBlock(
        CustomLayerNorm(embedding_dim; bias=ln_bias, eps=1f-5),
        MultiHeadAttention(
            embedding_dim;
            nheads=n_heads,
            bias=attention_bias,
...
function (m::_TransformerBlock)(q::AbstractArray{Float32, 3})
    a = m.layer_norm_pre(q)
    a, _ = m.multi_head_attention(a; mask=make_causal_mask(q))
...
"""
    (::GPT)(q::AbstractArray{Float32, 3})

Forward pass of the GPT model.

Args:
  * q: Queries from zero to current time step or set of queries, with shape
        (`observation_dim`, `0:t` or sequence_length`, `batch_size`).

Returns:
  * Output of the transformer of shape (`embedding_dim`, `0:t` or
        `sequence_length`, `batch_size`).
"""
function (m::GPT)(q::AbstractArray{Float32, 3})
    x = m.embedding_dropout(
        m.token_embedding(q) .+ m.position_embedding(1:size(q, 2))
    )
